<?php
class Qss_View_Button_M759_Employee_Plan_Task extends Qss_View_Abstract
{
	...
	public function __doExecute ($user,$m,$y,$workcenter)
	{
        ...
        $dvbt = $model->getTaskWorkCenter($startdate, $enddate);
        
        foreach($dvbt as $item)
        {
        	$arrDVBT[$item->IOID][$item->NgayBatDauDuKien] = array('TongSo'=>$item->TongSo,'DaGiao'=>$item->DaGiao);
        }
        $data = $model->getCountTaskByEmployee($startdate,$enddate,$workcenter);
