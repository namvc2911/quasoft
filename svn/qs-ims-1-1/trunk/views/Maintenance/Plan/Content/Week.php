<?php
// @todo: Cần tính thêm phần runtime như trong PMNotify.php, có thể phải xét theo lịch làm việc
// @todo: Chỉ số chưa tính trường hợp (cả chỉ số lẫn định kỳ), Chỉ số chưa tính với kỳ lặp
class Qss_View_Maintenance_Plan_Content_Week extends Qss_View_Abstract
{
	...
	public function __doExecute ($fid, $user, $w, $m, $y, $workcenter, $responseid, $assignid, $locationioid)
	{
        if($w == 53 && $m == 1)
        {
            $y--;
		...
		$this->html->data       = $this->_getCalendar($startdate, $enddate, $locationioid);
     ...
     */
    private function _getCalendar($startDate, $endDate, $locIOID)
    {
        // Phieu bao tri
        //$this->_getExistsOrder($startDate, $endDate);

        // Them ke hoach bao tri dinh ky
        $this->_addBasicPlan($startDate, $endDate, $locIOID);
    ...
    private function _addBasicPlan($startDate, $endDate, $locIOID= 0)
    {
        ...
        while ($tStartDate <= $cEndDate)
        {
            ...
            $plans 	= $basicplan->getPlansByDate($date,$locIOID);
    ...
    private function _sortPlans($startDate, $endDate)
    {
        ...
        foreach ($this->plans as $key=>$item)
        {
            ...
            while(1)
            {
                ...
                {
                    if(isset($this->calendar[$item->Ref_DVBT]['Data'][$i][$j]))
                    {
                        $ok = false;
                    }
                ...
                if($ok)
                {
                    for($j=$startcol;$j<=$endcol;$j++)
                    {
                       ...
                       $this->calendar[$item->Ref_DVBT]['Name'] = $item->TenDVBT;
                       $this->calendar[$item->Ref_DVBT]['Data'][$i][$j] = $add;
                    }
                    break;
                }
                $i++;
