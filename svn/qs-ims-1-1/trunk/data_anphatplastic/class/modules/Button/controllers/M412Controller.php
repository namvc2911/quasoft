...
class Button_M412Controller extends Qss_Lib_Controller
{
    ...
    public function materialSelectAction()
    {
        ...
        $filter   = $this->params->requests->getParam('m412_material_filter', 0);
        $ifid     = $this->params->requests->getParam('ifid', 0);
        $item     = $this->params->requests->getParam('itemSuggestion', '');

        $this->html->stocks       = ($filter != 1)?$mStock->getAll($this->_user):array();
        $this->html->filter       = $filter;
        $this->html->underminimum = ($filter == 1)?$mRequest->getUnderMinimumItems($this->_user, $ifid):array();
        // $this->html->items        = ($filter != 1)?$mRequest->getInventoryOfItems($item, $ifid):array();
    }

    /**
     * Lấy giới hạn mặt hàng cho trường hợp chọn mặt hàng
     */
    public function materialItemsAction()
    {
        $mRequest = new Qss_Model_Purchase_Aprequest();
        $ifid     = $this->params->requests->getParam('ifid', 0);
        $item     = $this->params->requests->getParam('itemSuggestion', 0);
        $this->html->items = $mRequest->getInventoryOfItems($item, $ifid);
    }
