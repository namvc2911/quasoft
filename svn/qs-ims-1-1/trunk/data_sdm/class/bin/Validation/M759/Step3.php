...
class Qss_Bin_Validation_M759_Step3 extends Qss_Lib_Warehouse_WValidation
{
	...
	public function onNext()
	{
		...
		// Điền thời gian hoàn thành
        $this->addFinishDate();

		// Ngày kết thúc phải nhỏ hơn ngày hiện tại
...
//		}
	...
	private function addFinishDate() {
        $model  = new Qss_Model_Import_Form('M759', false, false);
	    $insert = array();

        if(!$this->_params->Ngay) {
            $insert['OPhieuBaoTri'][0]['ifid']       = $this->_params->IFID_M759;
            $insert['OPhieuBaoTri'][0]['Ngay']       = date('Y-m-d');
        }

        if(!$this->_params->GioKetThuc) {
            $insert['OPhieuBaoTri'][0]['ifid']       = $this->_params->IFID_M759;
            $insert['OPhieuBaoTri'][0]['GioKetThuc'] = date('H:i:s');
        }
