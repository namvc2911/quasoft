...
class Static_M745Controller extends Qss_Lib_Controller
{
    ...
    public function excelAction()
    {
        ...
        $file       = new Qss_Model_Excel(Qss_Lib_System::getTemplateFile('M745', 'SHIV_BaoCaoSuCoTheoNguyenNhan.xlsx'));
        $mBreakdown = new Qss_Model_Maintenance_Breakdown();;
        $start      = $this->params->requests->getParam('start', '');
        $end        = $this->params->requests->getParam('end', '');
        ...
        $tableTitle->k   = $fields->getFieldByCode('BoPhan')->szFieldName;
        $tableTitle->l   = $this->html->_translate(4);
        $tableTitle->m   = $fields->getFieldByCode('NguyenNhan')->szFieldName;
        $tableTitle->n   = $fields->getFieldByCode('BienPhapKhacPhuc')->szFieldName;
        $tableTitle->o   = $fields->getFieldByCode('DoiSach')->szFieldName;
        $tableTitle->p   = $this->html->_translate(3);
        ...
        foreach ($reports as $item)
        {
            ...
            $sub->k = $item->BoPhan;
            $sub->l = @$item->MoTa;
            $sub->m = @$item->NguyenNhan;
            $sub->n = @$item->BienPhapKhacPhuc;
            $sub->o = @$item->DoiSach;
            $sub->p = '';
