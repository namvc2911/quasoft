...
function rowEventInsert() {
	//var url = sz_BaseUrl + '/static/m729/edit';
	//window.location.href = url;
	var url = 'http://' + location.host + '/static/m729/edit';
	var data = {};
...
function rowEventEdit(id) {
	...
	var url = sz_BaseUrl + '/static/m729/edit?id=' + row.id;
	window.location.href = url;*/
	var url = 'http://' + location.host + '/static/m729/edit';
	var data = {id:id};
	qssAjax.getHtml(url, data, function(jreturn) {
...
function rowEventRefer() {
	...
	var url = sz_BaseUrl + '/static/m729/refer?id=' + row.id;
//	window.location.href = url;
        openModule('', url);
}
function rowEventDelete() {
	...
	qssAjax.confirm(Language.translate('CONFIRM_DELETE'),function(){
		...
		var url = sz_BaseUrl + '/static/m729/delete';
...
function gotoImport(id,fid,objid){
	var url = sz_BaseUrl + '/static/m729/import?eventid='+id+'&fid='+fid+'&objid='+objid;
//	window.location.href = url;
        openModule('', url);
}
function showSearch() {
...
function importSearch(fid, ifid, deptid, objid) {
	...
	var url = sz_BaseUrl + '/static/m729/import/reload?fid=' + fid + '&ifid=' + ifid
...
function objectImport(fid, ifid, deptid, objid) {
	var eventid = $('#eventid').val();	
	var url = sz_BaseUrl + '/static/m729/import/import';
	var ioidlist = '';
	var fidlist = '';
...
function eventReferBack() {
	var eventid = $('#eventid').val();	
	var url = sz_BaseUrl + '/static/m729/refer?id='+eventid;
//	window.location.href = url;
        openModule('', url);
}
function rowEventBack(id) {
	var url = sz_BaseUrl + '/static/m729?id='+id;
//	window.location.href = url;
        openModule('', url);
}
function eventAction(ele,id, action) {
	if(!$(ele).hasClass('disabled')){
		var url = sz_BaseUrl + '/static/m729/action';
		var data = {
			id : id,
			action : action
		};
...
function eventReferDelete(fid,objid,id,ifid,ioid) {
	qssAjax.confirm(Language.translate('CONFIRM_DELETE'),function(){
		var url = sz_BaseUrl + '/static/m729/refer/delete';
		var data = {
				fid : fid,
				objid : objid,
				id : id,
...
function reloadEventRefer(id,fid, objid) {
	...
	var url = sz_BaseUrl + '/static/m729/refer/reload';
	var data = {id:id,
				fid:fid,
				objid:objid};
	qssAjax.getHtml(url, data, function(jreturn) {
...
function rowSearch() {
	...
	var perpage = $('#qss_form_perpage').val();
	var groupby = $('#qss_form_groupby').val();
	var url = sz_BaseUrl + '/static/m729/grid';
...
function eventLogDelete(ele,elid) {
	if(!$(ele).hasClass('disabled')){
		qssAjax.confirm(Language.translate('CONFIRM_DELETE'),function(){
			var url = sz_BaseUrl + '/static/m729/log/delete';
			var data = {elid:elid};
			qssAjax.call(url, data, function(jreturn) {
				rowSearch();
			}, function(jreturn) {
...
function eventNoteSave(elid) {
	var url = sz_BaseUrl + '/static/m729/note/save';
...
function downloadTemplate(id) {
	var url = sz_BaseUrl + '/static/m729/download?id=' + id;
//	window.location.href = url;
        openModule('', url);
}
function deleteTemplate(id) {
	qssAjax.confirm(Language.translate('CONFIRM_DELETE_TEMPLATE'),function(){
		var url = sz_BaseUrl + '/static/m729/delete/template?id=' + id;
		var data = {id:id};
		qssAjax.call(url, data, function(jreturn) {
...
function showMonth(m,y) {
	...
	var url = sz_BaseUrl + '/static/m729/reload';
	if(m) $('#month').val(m);
	if(y) $('#year').val(y);
	$('#type').val('month');
...
function createCalendar() {
	var url = sz_BaseUrl + '/static/m729/create';
	var data = {};
	qssAjax.getHtml(url, data, function(jreturn) {
...
{
	var url  = sz_BaseUrl + '/static/m729/saveplanworkorder';
...
{
	var url  = sz_BaseUrl + '/static/m729/saveworkorder';
...
{
	var url = sz_BaseUrl + '/static/m729/tasks';
	var data = {ifid:ifid,deptid:deptid, module:module} ;
	data=  $.extend({}, data, dat);
	qssAjax.getHtml(url, data, function(jreturn) {
