...
function rowEventInsert() {
	//var url = sz_BaseUrl + '/user/event/edit';
	//window.location.href = url;
	var url = 'http://' + location.host + '/user/event/edit';
	var data = {};
...
function gotoImport(id,fid,objid){
	var url = sz_BaseUrl + '/user/event/import?eventid='+id+'&fid='+fid+'&objid='+objid;
//	window.location.href = url;
        openModule('', url);
}
function showSearch() {
...
function importSearch(fid, ifid, deptid, objid) {
	...
	var url = sz_BaseUrl + '/user/event/import/reload?fid=' + fid + '&ifid=' + ifid
...
function objectImport(fid, ifid, deptid, objid) {
	var eventid = $('#eventid').val();	
	var url = sz_BaseUrl + '/user/event/import/import';
	var ioidlist = '';
	var fidlist = '';
...
function eventReferBack() {
	var eventid = $('#eventid').val();	
	var url = sz_BaseUrl + '/user/event/refer?id='+eventid;
//	window.location.href = url;
        openModule('', url);
}
...
function eventAction(ele,id, action) {
	if(!$(ele).hasClass('disabled')){
		var url = sz_BaseUrl + '/user/event/action';
		var data = {
			id : id,
			action : action
		};
...
function eventReferDelete(fid,objid,id,ifid,ioid) {
	qssAjax.confirm(Language.translate('CONFIRM_DELETE'),function(){
		var url = sz_BaseUrl + '/user/event/refer/delete';
		var data = {
				fid : fid,
				objid : objid,
				id : id,
...
function reloadEventRefer(id,fid, objid) {
	...
	var url = sz_BaseUrl + '/user/event/refer/reload';
	var data = {id:id,
				fid:fid,
				objid:objid};
	qssAjax.getHtml(url, data, function(jreturn) {
...
function rowSearch() {
	...
	var perpage = $('#qss_form_perpage').val();
	var groupby = $('#qss_form_groupby').val();
	var url = sz_BaseUrl + '/user/event/grid';
...
function eventLogDelete(ele,elid) {
	if(!$(ele).hasClass('disabled')){
		qssAjax.confirm(Language.translate('CONFIRM_DELETE'),function(){
			var url = sz_BaseUrl + '/user/event/log/delete';
			var data = {elid:elid};
			qssAjax.call(url, data, function(jreturn) {
				rowSearch();
			}, function(jreturn) {
...
function eventNoteSave(elid) {
	var url = sz_BaseUrl + '/user/event/note/save';
...
function deleteTemplate(id) {
	qssAjax.confirm(Language.translate('CONFIRM_DELETE_TEMPLATE'),function(){
		var url = sz_BaseUrl + '/user/event/delete/template?id=' + id;
		var data = {id:id};
		qssAjax.call(url, data, function(jreturn) {
...
function showMonth(m,y) {
	...
	var url = sz_BaseUrl + '/user/event/reload';
	if(m) $('#month').val(m);
	if(y) $('#year').val(y);
	$('#type').val('month');
