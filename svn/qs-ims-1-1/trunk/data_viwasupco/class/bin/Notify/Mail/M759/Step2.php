<?php
class Qss_Bin_Notify_Mail_M759_Step2 extends Qss_Lib_Notify_Mail
{
	const TITLE ='Gửi email khi ban hành phiếu bảo trì';
	...
	public function __doExecute($user,$status,$comment)
	{
        ...
        if($loaiBaoTriCoGuiMail)
        {
            ...
            // GỬI EMAIL CHO NGƯỜI THỰC HIỆN
            $mEmailList = Qss_Model_Db::Table('qsusers');
            $mEmailList->select('qsusers.EMail, qsusers.UserName');
            $mEmailList->join(' inner join ODanhSachNhanVien on qsusers.UID = ODanhSachNhanVien.Ref_TenTruyCap ');
            $mEmailList->join(' inner join OCongViecBTPBT on ODanhSachNhanVien.IOID = OCongViecBTPBT.Ref_NguoiThucHien ');
            $mEmailList->where(sprintf('OCongViecBTPBT.IFID_M759 = %1$d', $this->_params->IFID_M759));
            $mEmailList->where($mEmailList->ifnullNumber('qsusers.isActive', 1));
            ...
            {
                $subject  = '[QS-M759] Ban hành phiếu ';
