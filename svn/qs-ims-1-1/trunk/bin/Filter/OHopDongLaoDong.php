<?php
class Qss_Bin_Filter_OHopDongLaoDong extends Qss_Lib_Filter {
    ...
    public function getRights($ifid) {
        ...
        if(Qss_Lib_System::formSecure('M319')) {
            $sql = sprintf('
                ...
                left join OPhongBan on OHopDongLaoDong.PhongBanHienTai = OPhongBan.MaPhongBan
                left join qsrecordrights on OPhongBan.IFID_M319 = qsrecordrights.IFID and qsrecordrights.UID = %1$d 
                WHERE OHopDongLaoDong.IFID_M318 = %2$d'
                ,$this->_user->user_id
                ,$ifid);
