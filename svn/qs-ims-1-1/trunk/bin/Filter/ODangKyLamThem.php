<?php
class Qss_Bin_Filter_ODangKyLamThem extends Qss_Lib_Filter {
    ...
    public function getRights($ifid) {
        ...
        if(Qss_Lib_System::formSecure('M319')) {
            $sql = sprintf('
                ...
                from ODangKyLamThem
                left join OPhongBan on ODangKyLamThem.PhongBanHienTai = OPhongBan.MaPhongBan
                left join qsrecordrights on OPhongBan.IFID_M319 = qsrecordrights.IFID and qsrecordrights.UID = %1$d 
                WHERE ODangKyLamThem.IFID_M078 = %2$d'
                ,$this->_user->user_id
                ,$ifid);
