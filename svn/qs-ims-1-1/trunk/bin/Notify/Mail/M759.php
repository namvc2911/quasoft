<?php
class Qss_Bin_Notify_Mail_M759 extends Qss_Lib_Notify_Mail
{
	...
	public function sendComment($user,$comment)
	{
		$toMails = array();
		$ccMails = array();
		...
		{
			if($user->user_id != $item->UID)
			{
				$toMails[$item->EMail] = $item->UserName;
			}
		}
		$sql = sprintf('select ODonViSanXuat.*, qsusers.EMail,qsusers.UserName
							from ODonViSanXuat 
							inner join ONhanVien on ONhanVien.IFID_M125 = ODonViSanXuat.IFID_M125
							inner join ODanhSachNhanVien on ODanhSachNhanVien.IOID = ONhanVien.Ref_MaNV
							inner join qsusers on qsusers.UID =  ODanhSachNhanVien.Ref_TenTruyCap
		...
		if(count($toMails))
		{
			...
			$body = sprintf('Chào bạn!
				 <p>%1$s đã cho ý kiến trên %2$s %7$s
				 <strong>"%6$s"</strong>
