...
// @Remove
class Qss_Bin_Bash_CreateDefect extends Qss_Lib_Bin
{
        ...
        public function __doExecute()
        {
                ...
                $manuLineInfo = $extra->getTable(array('*'), 'ODayChuyen',
                        array('IOID' => $this->_params->Ref_DayChuyen), array(),
                        1, 1);
                ...
                foreach ($this->_params->OSanLuong as $item)
                {
                        ...
                        $insert['OHangDoiNhap'][0]['Kho'] = $manuLineInfo ? $manuLineInfo->KhoPheLieu : '';
                        $insert['OHangDoiNhap'][0]['MoTa'] = $this->_params->MaLenhSX;

                        $service = $this->services->Form->Manual('M610', 0,
                                $insert, false);
                        if ($service->isError())
                        {
                                $this->setError();
