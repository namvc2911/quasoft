...
class Qss_Service_Extra_Warehouse_Input_Save extends Qss_Service_Abstract
{
	...
	{		
		...
		if($this->validate($params))
		{
			...
			foreach ($line as $itemCode)
			{
				...
				$insert['ODanhSachNhapKho'][$ai]['MaSanPham']   = $itemCode;
				$insert['ODanhSachNhapKho'][$ai]['SoLuong']     = $params['qty'][$ai];
                                $insert['ODanhSachNhapKho'][$ai]['DonViTinh']   = $params['itemUOM'][$ai];
				$insert['ODanhSachNhapKho'][$ai]['DongDonHang'] = $info?$info->docNo:'';
				$insert['ODanhSachNhapKho'][$ai]['ThuocTinh']   = $params['attributeCode'][$ai];
				$insert['ODanhSachNhapKho'][$ai]['DonGia']      = $params['price'][$ai];
				$insert['ODanhSachNhapKho'][$ai]['ioidlink']    = $params['stackLine'][$ai];
	...
	private function validate($params)
	{
		...
		{
			$return = false;
			$this->setMessage('Không thể xóa do nhận hàng đã được hoàn thành!');
		}
		return $return;
	}
}
