<?php
class Qss_Service_Inventory_Output_SavePositions extends Qss_Lib_Service
{
    ...
    public function __doExecute ($params)
    {
        ...
        $i      = 0;
        $insert = array();
        $ss     = $mInv->getOutputStockStatus(@(int)$params['ifid']);
        ...
        if(count($remove))
        {
            $service = $this->services->Form->Remove('M506',  $params['ifid'],  $remove, false);
            if ($service->isError())
            {
                $this->setError();
        ...
        {
            $service = $this->services->Form->Manual('M506',  $params['ifid'],  $insert, false);
            if ($service->isError())
            {
                $this->setError();
                $this->setMessage($service->getMessage(Qss_Service_Abstract::TYPE_TEXT));
