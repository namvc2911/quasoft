 ...
 *
 */
class User_BashController extends Qss_Lib_Controller
{
	protected $_model;
	protected $_form;
	...
	public function init ()
	{
		...
		$this->_form->v_fInit(23,$this->_user->user_dept_id,$this->_user->user_id);
	}
	...
	{
		if($this->b_fCheckRightsOnForm($this->_form,4))
		{
	...
	public function editAction ()
	{
		if($this->b_fCheckRightsOnForm($this->_form,2))
		{
			...
			$this->html->forms = $form->a_fGetAll();
			$fid = @$this->html->bash->FID;
			if($fid)
			{
				if($form->v_fInit($fid, $this->_user->user_dept_id, $this->_user->user_id))
				{
	...
	{
		$id = $this->params->requests->getParam('id',0);
		if($this->b_fCheckRightsOnForm($this->_form,8))
		{
	...
	{
		if($this->b_fCheckRightsOnForm($this->_form,16))
		{
			$id = $this->params->requests->getParam('id', 0);
