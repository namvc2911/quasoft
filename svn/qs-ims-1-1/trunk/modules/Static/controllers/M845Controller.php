...
class Static_M845Controller extends Qss_Lib_Controller
{
    ...
    public function showAction()
    {
        ...
        $this->html->report = $mPartner->getContactsOfPartners($partner, true, false);
        $this->html->count  = $mPartner->countContactsOfPartners($partner, true, false);
    }

    public function excelAction()
    {
        header("Content-type: application/vnd.openxmlformats-officedocument.spreadsheetml.sheet");
        header("Content-disposition: attachment; filename=\"Danh sách nhà cung cấp dịch vụ.xlsx\"");

        $mPartner = new Qss_Model_Master_Partner();
        $partner = $this->params->requests->getParam('partner', 0);

        $report = $mPartner->getContactsOfPartners($partner, true, false);
        $count = $mPartner->countContactsOfPartners($partner, true, false);
        $stt = 0;
        $row = 8;
        $merge = array();
