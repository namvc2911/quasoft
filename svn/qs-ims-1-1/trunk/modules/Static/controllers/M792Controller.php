...
class Static_M792Controller extends Qss_Lib_Controller
{
	...
	public function getMaintainPlanByYear($year, $locIOID = 0, $eqIOID = 0, $eqTypeIOID = 0, $eqGroupIOID = 0 , $start = '', $end = '', $workcenter= 0)
	{
		$retval      = array();
		$retval2     = array();
		$planIFIDArr = array();
	...
	public function getElementForPlanYearReprot($year, $pl, $retval2)
	{
		...
		if( in_array($pl->MaKy, $appovedPeriod))
		{
			...
			if($pl->MaKy == Qss_Lib_Extra_Const::PERIOD_TYPE_MONTHLY)
			{
				...
				$first = ($lap==1)?$tempFirst:($pl->NgayBatDau?$pl->NgayBatDau:$tempFirst);
...
//                continue;
//            }
			//chỉnh lại first nếu năm bắt đầu trước năm in kế hoạch
			while (1 && $yearOfFirst < $year)
			{
				
				if($pl->MaKy == Qss_Lib_Extra_Const::PERIOD_TYPE_WEEKLY)
				{
					$first = date("Y-m-d", strtotime("+".$lap." weeks", strtotime($first)));
				}
				elseif($pl->MaKy == Qss_Lib_Extra_Const::PERIOD_TYPE_MONTHLY)
				{
					$first = date("Y-m-d", strtotime("+".$lap." months", strtotime($first)));
				}
				elseif($pl->MaKy == Qss_Lib_Extra_Const::PERIOD_TYPE_YEARLY)
				{
					$first = date("Y-m-d", strtotime("+".$lap." years", strtotime($first)));
				}
				$yearOfFirst = date('Y', strtotime($first));
				if($yearOfFirst >= $year)
				{
			...
			if($yearOfFirst <= $year)
			{
				...
				//$first = $tempFirst; sao lại gắn lại ở đây
				...
				while ($tempStart <= $lastDayTmp)
				{
					...
					if($pl->MaKy == Qss_Lib_Extra_Const::PERIOD_TYPE_WEEKLY)
					{
						$next = date("Y-m-d", strtotime("+".$lap." weeks", strtotime($next)));
					}
					elseif($pl->MaKy == Qss_Lib_Extra_Const::PERIOD_TYPE_MONTHLY)
					{
